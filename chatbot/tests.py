...
Sends a message to api_chat via a POST request.
...
class ApiChatTestCase(TestCase):
    ...
    def test_api_chat_response_and_db_save(self):
        # Mock Rasa's response
        rasa_response = [{"text": "Hello from Rasa!"}]
        with requests_mock.Mocker() as m:
            m.post("http://localhost:5005/webhooks/rest/webhook", json=rasa_response)

            # Send message to api_chat
            response = self.client.post(reverse('chatbot:api_chat'), data={"message": "Hello"})
            ...
            self.assertEqual(saved_conversation.user_message, "Hello")
